...
class Linear(BaseNN):
    ...
    def init_params(self, input_dims):
        ...
        self.norm_gamma = np.zeros((self.node_size, 1)) * scaler

    def update(self):
        if self.norm == "l2":
            reg = self.lambd/self.grads['W'].shape[1] * \
                np.linalg.norm(self.weights)

        elif self.norm == "l1":
            reg = self.lambd/self.grads['W'].shape[1] * \
                np.linalg.norm(self.weights, ord=1)
        ...
        if self.optimiser == "GDMomentum":
            ...
            self.weights = self.weights - self.lr * \
                self.grads['W']/np.sqrt(self.S_w+self.eps) + reg
            self.bias = self.bias - self.lr * \
                self.grads['b']/np.sqrt(self.S_b+self.eps)

        elif self.optimiser == "adam":
            self.V_w = self.beta_v*self.V_w + (1-self.beta_v)*self.grads['W']
            self.V_b = self.beta_v*self.V_b + (1-self.beta_v)*self.grads['b']

            self.S_w = self.beta_s*self.S_w + \
                (1-self.beta_s)*self.grads['W']**2
            self.S_b = self.beta_s*self.S_b + \
                (1-self.beta_s)*self.grads['b']**2
            ...
            self.bias = self.bias - self.lr * \
                V_b_corr/np.sqrt(S_b_corr+self.eps)
...
class InvertedDropout():
    ...
    def forward(self, A):
        self.rand_init = np.random.rand(
            A.shape[0], A.shape[1]) < self.keep_prob
